...
export function Layout() {
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    if (!usernameRegex.test(profUsername)) {
      setProfError('Username must be lowercase, start with a letter/number, and contain only allowed symbols.');
      return;
    }
  ...
  return (
    <AuthRoute>
      <div className="flex min-h-screen bg-gray-900 font-sans text-gray-100 overflow-hidden relative">
        {/* Mobile Overlay */}
        {mobileMenuOpen && (
          <div 
        ...
        {showProfileModal && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
            <div className="bg-gray-800 rounded-2xl border border-gray-700 w-full max-w-md shadow-2xl overflow-hidden relative animate-in fade-in zoom-in duration-300">
              <button 
              ...
              <div className="p-6 sm:p-8">
                <div className="mb-6 text-center">
                  ...
                  <p className="text-sm text-gray-400">
                    Welcome! We noticed you don't have a username yet. Please set up your profile to enable username login.
                  </p>
                </div>
                ...
                <form onSubmit={handleProfileSubmit} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-1.5">First Name <span className="text-red-500">*</span></label>
                      <input 
                        type="text" 
                        required 
                        value={profFirstName}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-1.5">Last Name <span className="text-red-500">*</span></label>
                      <input 
                        type="text" 
                        required 
                        value={profLastName}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1.5">Username <span className="text-red-500">*</span></label>
                    <input 
                      type="text" 
                      required 
                      value={profUsername}
                      onChange={(e) => setProfUsername(e.target.value.toLowerCase())}
                      className="w-full bg-gray-900 border border-gray-600 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-blue-500" 
                      placeholder="johndoe"
                    />
                    <p className="text-xs text-gray-500 mt-1.5">Only lowercase letters, numbers, and ._-@</p>
                  </div>
                  ...
                  <div className="pt-2">
                    ...
                    <button 
                      type="button" 
                      onClick={handleDismissModal}
                      className="w-full mt-2 bg-transparent text-gray-400 hover:text-white font-medium py-2 rounded-lg transition-colors text-sm"
                    >
                      I'll do this later
                    </button>
                  </div>
